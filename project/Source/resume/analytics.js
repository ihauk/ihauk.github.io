...
function tracker_form(){
	if(querySV!='-'){
		...
		for(i=0;f[i];i++) if(f[i].method.toLowerCase()=="get"){
			for(j=0;j<f[i].length;j++) if(f[i].elements[j].name==query_name){flagEle=true;break;}
			if(!flagEle){
				newHidden = document.createElement("input");
				newHidden.name=query_name;
				newHidden.type="hidden";
				newHidden.value=querySV;
				f[i].appendChild(newHidden);
			}
		}
...
function wam_FixC(c,s,t) {
	...
	//if(value=='-') value=wam_GC(c,cookie_name2+"=",s);
 	var lt=0,i=0;
...
var firstCookieN = "firstchannelurl";
var lastCookieN = "lastchannelurl";
var firstlast_cp = "/";
var firstlast_dn="zhaopin.com";
function firstChannelURL_cookie(){
	if(wam_dl.protocol=="file:") return;
	var exp = new Date();
	var nowPlusOneDay = exp.getTime() + (24*60*60*1000);
	exp.setTime(nowPlusOneDay);
	var nx=" expires="+exp.toGMTString()+";";
	var index;
	var dc=wam_d.cookie;
	index=dc.indexOf(firstCookieN+"=");
	if(index==-1) wam_d.cookie=firstCookieN+"="+escape(wam_dl)+"; path="+firstlast_cp+";"+nx+" domain="+firstlast_dn+";";
}

function lastChannelURL_cookie(){
	if(wam_dl.protocol=="file:") return;
	var exp = new Date();
	var nowPlusOneDay = exp.getTime() + (24*60*60*1000);
	exp.setTime(nowPlusOneDay);
	...
	var index;
	var dc=wam_d.cookie;
	index=dc.indexOf(lastCookieN+"=");
	var flag = wam_dl.pathname.toLowerCase().indexOf('regist_serviceitem.asp')>-1?false:true;
...
function urlEncode(v){
	return window.encodeURIComponent?encodeURIComponent(v):escape(v);
}
function urlDecode(v){
	return window.decodeURIComponent?decodeURIComponent(v):unescape(v);
}